...
pair<int, int> dijkstraUsingHeap(const unordered_map<int, vector<int>>& graph, int startNode) {
    using FH = FibonacciHeap<int, int>;
    unordered_map<int, int> distances;                      // Distance from startNode to each node
    unordered_map<int, FH::Entry> nodeMap; // Map to store heap nodes
    FH heap;
    ...
    nodeMap[startNode] = FH::makeEntry(0, 1); // Insert startNode with distance 0
    heap.insert(nodeMap[startNode]);
    ...
    while (true) {
        ...
        if (heap.size() == 0)
            break;
        int currentDist = (heap.extractMin())->key();
        ...
        for (int neighbor : graph.at(currentNode)) {
            ...
            if (newDist < distances[neighbor]) {
                ...
                if (nodeMap[neighbor]) {
                    ...
                    nodeMap[neighbor] = FH::makeEntry(newDist, 1);
                    heap.insert(nodeMap[neighbor]);
                }
