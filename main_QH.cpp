...
pair<int, int> dijkstraUsingHeap(const unordered_map<int, vector<int>>& graph, int startNode) {
    ...
    unordered_map<int, QH::Entry> nodeMap; // Map to store heap nodes
    QH heap;
    ...
    nodeMap[startNode] = QH::makeEntry(0, 1); // Insert startNode with distance 0
    ...
    while (true) {
        ...
        if (heap.size() == 0)
            break;
        int currentDist = (heap.deleteMin())->key();
        ...
        for (int neighbor : graph.at(currentNode)) {
            ...
            if (newDist < distances[neighbor]) {
                ...
                if (nodeMap[neighbor]) {
                    ...
                    nodeMap[neighbor] = QH::makeEntry(newDist, 1);
